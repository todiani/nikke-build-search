...
// @version      34.9
// @description  v34.9: 소장품/애장품 등급(SSR/SR/R) 추출 로직 보완
...
(function() {
    ...
    function createUI() {
        ...
        header.innerHTML = '<b style="color:#FF0050">NIKKE</b> EXTRACTOR <small style="opacity:0.6">v34.9</small>';
    ...
    const extractData = async () => {
        ...
        const clickTab = async (tabName) => {
            const elements = Array.from(document.querySelectorAll('div, span, button, a, .van-tab__text'));
            const targetTab = elements.find(el => el.innerText.trim() === tabName);
            if (targetTab) {
                targetTab.click();
        ...
        await clickTab('큐브');
        await new Promise(r => setTimeout(r, 1200)); // 큐브 탭 데이터 로딩 대기 강화
        
        let cubeLevel = 0;
        // 큐브 레벨 추출을 위한 정밀 함수 (LV.10, LV 10, 10 등 다양한 형식 대응)
        const parseCubeLv = (el) => {
            if (!el) return 0;
            // 내부 텍스트 전체를 합쳐서 숫자 추출 (LV. 3, LV3, 3 등)
            const text = el.innerText.replace(/\s+/g, '').toUpperCase();
            const match = text.match(/LV\.?(\d+)/i) || text.match(/(\d+)/);
            return match ? parseInt(match[1]) : 0;
        };

        const rubikItems = Array.from(document.querySelectorAll('.rubik-item'));
        
        // 모든 아이템(전투/아레나 등)에서 레벨 후보 수집
        const candidates = rubikItems.map(item => {
            const nameEl = item.querySelector('.rubik-name');
            const lv = parseCubeLv(nameEl) || parseCubeLv(item);
            return {
                isCombat: item.innerText.includes('전투'),
                level: lv
            };
        }).filter(c => c.level > 0);

        // 1. 전투 레이어 레벨이 있으면 최우선
        const combatCandidate = candidates.find(c => c.isCombat);
        if (combatCandidate) {
            cubeLevel = combatCandidate.level;
        } 
        // 2. 전투 레이어에 레벨이 없으면 아레나 등 다른 레이어에서 추출
        else if (candidates.length > 0) {
            cubeLevel = candidates[0].level;
        }
        ...
        // 1. 모든 div/span에서 SSR, SR, R 검색 (가장 정확한 직접 매칭)
        const possibleRarity = Array.from(document.querySelectorAll('div, span'))
            .find(el => {
                const txt = el.innerText.trim();
                return (txt === 'SSR' || txt === 'SR' || txt === 'R') && el.offsetParent !== null;
            });
        
        if (possibleRarity) {
            collectionRarity = possibleRarity.innerText.trim().toUpperCase();
        } else {
            // 2. 텍스트 패턴 매칭 보조
            const bodyText = document.body ? document.body.innerText : "";
            const rarityMatch = bodyText.match(/(?:소장품|애장품).*?\|\s*(SSR|SR|R)/i) || 
                                bodyText.match(/(SSR|SR|R)\s*(?:\(스킬Lv1|장비 능력치)/i);
            
            if (rarityMatch) {
                collectionRarity = rarityMatch[1].toUpperCase();
            } else if (bodyText.includes('애장품')) {
                collectionRarity = "SSR";
            }
        }

        const skillIcons = Array.from(document.querySelectorAll('div[data-cname="skill-icon"]'));
        const collSkillLevels = skillIcons.map(icon => {
            const span = icon.querySelector('span.text-highlight-blue.ff-num');
            return span ? parseInt(span.innerText.trim()) : 0;
        }).filter(val => !isNaN(val));
        const collection = { rarity: collectionRarity, skillLv1: collSkillLevels[0] || 0, skillLv2: collSkillLevels[1] || 0 };

        await clickTab('장비');
        await new Promise(r => setTimeout(r, 1200)); // 장비 탭 데이터 로딩 대기
        ...
        equipBlocks.forEach((block,i)=>{
            ...
            // 오버로드 옵션 추출 (슬롯 위치 보존을 위해 옵션 1, 2, 3 패턴 검색)
            const opts=[];
            for(let slotIdx=1; slotIdx<=3; slotIdx++) {
                // 1. "옵션 X [이름] 수치" 형태
                const fullRegex = new RegExp(`(?:옵션\\s*)?${slotIdx}(?:\\s*번\\s*옵션|\\s*옵션|\\s*)[\\s\\n]*\\[([^\\]]+)\\][\\s\\n]*([\\d.]+\\%)`, 'i');
                const mFull = block.match(fullRegex);
                
                // 2. "옵션 X 효과없음" 또는 "옵션 X [효과없음]" 형태
                const emptyRegex = new RegExp(`(?:옵션\\s*)?${slotIdx}(?:\\s*번\\s*옵션|\\s*옵션|\\s*)[\\s\\n]*(?:효과없음|\\[효과없음\\])`, 'i');
                const mEmpty = block.match(emptyRegex);

                if(mFull) {
                    opts.push({ slot: slotIdx, name: mFull[1].trim(), value: mFull[2].trim() });
                } else if(mEmpty) {
                    opts.push({ slot: slotIdx, name: "옵션없음", value: "0.00%" });
                }
            }
            
            // 만약 "옵션 X" 텍스트가 없는 경우를 위한 하이브리드 방식 (순서대로 3개 추출)
            if (opts.length === 0) {
                // [이름] 수치% 형태 또는 '효과없음' 텍스트 검색
                const optRegex = /(?:\[([^\]]+)\][\s\n]*([\d.]+%))|(효과없음)/g;
                let m;
                let fallbackIdx = 1;
                while((m=optRegex.exec(block))!==null && fallbackIdx <= 3) {
                    if (m[1]) {
                        opts.push({ slot: fallbackIdx++, name: m[1].trim(), value: m[2].trim() });
                    } else if (m[3]) {
                        opts.push({ slot: fallbackIdx++, name: "옵션없음", value: "0.00%" });
                    }
                }
            }
    ...
    setInterval(() => {
        // UI가 없으면 생성 (SPA 페이지 이동 시 증발 방지)
        if (!document.getElementById('nikke-ui-container')) {
            createUI();
        }
        
        // URL 변경 감지 (자동화 시 다음 니케로 넘어갔는지 확인)
        if (location.href !== lastUrl) {
            lastUrl = location.href;
            if (storage.get(AUTO_KEY) === 'true') {
                console.log("🔄 URL 변경 감지 -> 수집 재개");
