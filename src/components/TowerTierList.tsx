...
import { LATEST_TIERS } from '../utils/nikkeDataManager';

import { codeTextColors, burstColors, classColors, companyColors, classNames, weaponColors, weaponNames } from '../utils/nikkeConstants';
...
export default function TowerTierList({ allNikkes, onSelectNikke, towerSquads = {}, onSaveSquads, openNikkeSelector }: TowerTierListProps) {
    ...
    const currentSquads = towerSquads[towerKey] || [];
            ...
            >
                <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 hover:border-nikke-red group h-full flex flex-col justify-between">
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <span className={`text-[12px] font-black shrink-0 ml-1 ${
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3 3xl:grid-cols-4 gap-4">
                            ...
                            <div className="bg-gray-900/60 border border-gray-800 rounded-lg p-1.5 hover:border-blue-500/50 hover:bg-gray-800/40 cursor-pointer group relative flex flex-col gap-1.5 shadow-md min-h-[70px] transition-all">
                                ...
                                <div className="flex items-center justify-between w-full">
                                    <div className="flex items-center gap-2">
                                        <div className={`flex items-center justify-center w-5 h-5 rounded shadow-sm ${
                                            sIdx === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' : 
                                ...
                                <div className="grid grid-cols-5 gap-1.5 w-full">
                                    {(editingSquadIdx === sIdx ? tempSquad! : squad).map((name, idx) => {
                                        ...
                                        return (
                                            <div 
                                                key={idx} 
                                                onClick={() => editingSquadIdx === sIdx && handleSlotClick(idx)}
                                                className="group/nikke relative h-full"
                                            >
                                                <div className={`bg-gray-800/50 border ${editingSquadIdx === sIdx ? 'border-dashed border-blue-500 cursor-pointer' : 'border-gray-700'} rounded-xl p-3 hover:border-nikke-red group transition-all duration-300 h-full flex flex-col justify-between`}>
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="flex flex-col min-w-0 flex-1">
                                                            <div className="flex items-baseline gap-1.5 flex-wrap">
                                                                <h3 className="font-bold text-white group-hover/nikke:text-nikke-red text-[12px] truncate">
                                                                    {nikke ? nikke.name : (name.split(' : ')[0] || 'SELECT')}
                                                                </h3>
                                                                {nikke?.name_en && (
                                                                    <span className="text-[9px] text-blue-400 font-medium truncate">
                                                                        {nikke.name_en}
                                                                    </span>
                                                                )}
                                                            </div>
                                                            {nikke?.extra_info && (
                                                                <span className="text-[10px] text-orange-400 font-medium mt-0.5 truncate leading-tight">
                                                                    {nikke.extra_info}
                                                                </span>
                                                            )}
                                                        </div>
                    ...
                    {(() => {
                        ...
                        const dbNikkes = allNikkes.filter(n => {
                            if (activeTab === 'corporate') {
                                ...
                                if (companyName === '필그림') {
                                    const overspecNikkes = ['레드 후드', '라피 : 레드 후드', '미하라 : 본딩 체인', '세이렌 : 리틀 머메이드'];
